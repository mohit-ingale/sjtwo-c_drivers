...
void my_gpio_set(struct IO_PORT_PIN *pin_set){
	pin_set->port->SET |= 1<<(pin_set->pin_num);
}

void my_gpio_clr(struct IO_PORT_PIN *pin_set){
	pin_set->port->CLR |= 1<<(pin_set->pin_num);
}

void my_gpio_toggle(struct IO_PORT_PIN *pin_set){
	pin_set->port->PIN ^= 1<<(pin_set->pin_num);
}

int my_gpio_get(struct IO_PORT_PIN *pin_get){
	return (pin_get->port)->PIN & (1 << (pin_get->pin_num));
}

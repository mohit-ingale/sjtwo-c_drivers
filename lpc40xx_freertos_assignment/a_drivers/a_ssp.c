...
void a_ssp_init(uint32_t ssp_interface, uint32_t clock_frequency_in_mhz) {
  if (ssp_interface == 2) {
    ...
    // LPC_SC->PCLKSEL |= (2 << 1);    //Divide clock by two so that spi will divide clock further by 2 and resultant
    // speed will be 24mhz which is standard for sd card and pcb design
